...
function ProfileCard({ name, path, role, photo }) {
    return (
        <motion.div
            ...
            whileTap={{ scale: 0.98 }}
        >
            <Link to={`/${path}`} className="block rounded-xl hover:border-gray-300 hover:shadow-md transition group bg-[#181818] backdrop-blur-sm overflow-hidden"> 
                <div className="flex items-center gap-4">
                    ...
                    >
                        {photo ? (
                            ...
                            <div className="h-14 w-14 rounded-full bg-indigo-100 text-indigo-300 flex items-center justify-center font-semibold">
                                {getInitials(name)}
                            </div>
                        )}
                    </motion.div>
